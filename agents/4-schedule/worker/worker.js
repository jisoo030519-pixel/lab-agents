...
const MAX_TOKENS = 3000;    // 답 길이 상한. 추론 모델은 생각에도 토큰을 쓴다 (실측 700~950). 쓴 만큼만 든다
...
- 연차 등록은 받습니다. "10/1 현지수 연차 추가해줘" 는 add_event 로 넣으세요.
  title 은 "현지수 연차" 처럼 이름을 붙이고, member 에 그 사람 이름만, time 은 비웁니다.
- **한 문장에 등록과 질문이 같이 와도 등록은 반드시 하세요.**
  "9/21 현지수 연차 추가해줘. 그리고 남은 연차 갯수 알려줘" → 먼저 add_event 로 등록하고,
  그 다음 "남은 연차는 담당자 화면에서 볼 수 있습니다" 를 덧붙이세요.
  답할 수 없는 부분이 있다고 해서 할 수 있는 부분까지 거절하면 안 됩니다.
- 잔여 일수(남은 연차가 며칠인지)만 이 자료에 없습니다 — 그 부분만 담당자 안내로 답하세요.
...
- "정리해줘 / 알려줘" 처럼 넓게 물으면 목록으로 한 줄씩, 다섯 줄 안쪽으로 답하세요.
  전부 나열하려고 오래 고민하지 마세요.
...
async function askAi(req, env, ctx, json) {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    if (!calls.length && !text) {
      try {
        // 사용자 차례를 두 번 연달아 붙이면 모델이 "이전 메시지가 깨져 있다" 고 답한다.
        // 실제로 그렇게 답한 적이 있어서, 질문 하나짜리 대화로 새로 만든다.
        const r2 = await env.AI.run(model, { max_tokens: MAX_TOKENS, ...extra,
          messages: [messages[0], { role: "user",
            content: q + " (생각은 짧게. 목록으로 한 줄씩, 다섯 줄 안쪽으로만 답하세요.)" }] });
        text = readAi(r2).text;
        r = r2;
