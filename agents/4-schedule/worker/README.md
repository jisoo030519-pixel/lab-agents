...
| 모델 | 날짜 | 답 | 질문당 |
|---|---|---|---|
| `@cf/google/gemma-4-26b-a4b-it` (채택) | 정확 | 자연스러운 한국어 | 60~100 뉴런 |
| `@cf/qwen/qwen3-30b-a3b-fp8` | 정확 | "2026-09-16 15:00에" 식 | 약 2배 |

자료(학회·발표·정기 일정·날짜표)가 약 5천 토큰이라 질문 하나에 **실측 60~100 뉴런**이 든다.
**하루 무료 10,000 뉴런 → 대략 100~150 질문.** 랩 11명이 하루에 쓰기엔 충분하다.
넘기기 전에 우리 쪽 상한(`DAILY_TOTAL` 120)에서 먼저 "오늘 한도" 안내가 뜨고 규칙 기반으로 떨어진다.
매일 09:00 (KST) 에 초기화된다. 무료 플랜이라 넘겨도 요금은 나가지 않는다 — 그날 에러가 날 뿐이다.

### 실측으로 잡은 문제

| 증상 | 원인 | 대책 |
|---|---|---|
| "새로고침해줘" → 빈 답 | 애매한 말에 생각만 1200토큰(82뉴런) | `reasoning_effort: "low"` + 새로고침은 한 줄로 답하라는 규칙 |
| "이번 주에 뭐 있어?" → 4번 중 1번 빈 답 | low 로도 생각에 700~950토큰, 상한 1200 에 걸림 | 상한 2000 + 비면 "답만 짧게" 로 한 번만 재질문 |
| "바꿔줘" 가 가끔 실패 | 지우고-다시-넣기 두 번을 거쳐야 했음 | `update_event` 도구로 같은 id 에서 값만 변경 |

위 대책 후 실제 AI 로 질문 14개를 돌려 **빈 답 0건** 확인했다.
...
한도는 `wrangler.toml` 의 `PER_IP_HOUR`(30), `DAILY_TOTAL`(120).
